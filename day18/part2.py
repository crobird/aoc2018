...
def main(args):
	...
	base = 537
	distance = 28
	boards = [None for x in range(distance)]
	for m in range(minutes):
		minute = m + 1
		board.next_minute()
		bnd = base + distance
		if minute >= base and minute < bnd:
			bstr = str(board)
			modval = minute - base
			print("Storing {} for index {}".format(board.tree_count * board.lumberyard_count, modval))
			boards[modval] = board.tree_count * board.lumberyard_count
		elif minute >= bnd:
			break

	tomod = minutes - base
	answer = boards[tomod % distance]
